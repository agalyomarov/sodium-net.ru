...
class JackpotController extends Controller
{
	...
	public function index(Request $r) {
		...
		return view('pages.jackpot', compact('room', 'rooms'));
	}
	
	public function newGame(Request $r) {
		$room = $r->get('room');
		if(\Cache::get('act.newgame.'.$room) > time()) return false;
		\Cache::forever('act.newgame.'.$room, time() + 6);
		
		DB::table('jackpot')->where('room', $room)->where('status', '<', 3)->update([
            'status' => 3
        ]);
		...
		if($countGame > 100) Jackpot::where(['room' => $room, 'status' => 3])->orderBy('id', 'asc')->limit(1)->delete();
		
        Jackpot::where('updated_at', '>=', Carbon::today()->addDays(4))->delete();
		JackpotBets::where('created_at', '>=', Carbon::today()->addDays(4))->delete();
		$this->newBank();
	...
	public function newBet(Request $r) {
		...
		try {
			...
			if($moneytick > ($option->max - $usersum)) {
				$this->redis->publish('message', json_encode([
					'user'  => $this->user->id,
			...
			if($lastBet) $ticketFrom = $lastBet->to + 1;
	...
	public function adminBet(Request $r) {
		...
		$game = Jackpot::where('room', $room)->orderBy('id', 'desc')->first();
		$moneytick = preg_replace('/[^0-9.]/', '', floor($r->get('sum')));
		
		$option = Rooms::where('name', $room)->first();
		
		$countBet = JackpotBets::where('room', $room)->where(['user_id' => $user->id, 'game_id' => $game->game_id])->count();
		$usersum = JackpotBets::where('room', $room)->where(['game_id' => $game->game_id, 'user_id' => $user->id])->sum('sum');
		...
		if($moneytick > $option->max) return [
            'success' => false,
            'type' => 'error',
            'msg' => 'Ставка больше максимальной'
        ];
		if($moneytick < $option->min) return [
            'success' => false,
            'type' => 'error',
	...
	public function sendMoney($room, $game_id) {
		...
		JackpotBets::where('room', $room)->where('game_id', $game->game_id)->update([
	...
	public function req() {
		$game = Jackpot::where('created_at', '>=', Carbon::today())->where('room', 'small')->where('game_id', '<', 13202)->get();
		foreach($game as $g) {
			$u = User::where('id', $g->winner_id)->first();
			$u->balance -= $g->winner_sum;
			$u->save();
		}
		return $game;
	}
        ...
        foreach($games as $game) {
			...
			if(isset($winner)) {
				$history[] = [
					...
					'price' => $game->price,
					'bets' => $this->getChancesOfGame($game->room, $game->game_id)
				];
			}
        }
		
        return view('pages.jackpotHistory', compact('rooms', 'history'));
    }
	...
	public function newBank() {
		...
		$sum = $jackpot_r1->price + $jackpot_r2->price + $jackpot_r3->price;
