...
class BattleController extends Controller
{
	...
	public function index(Request $r) {
		...
		$lastwins = Battle::orderBy('id', 'desc')->where('status', Battle::STATUS_FINISHED)->limit(16)->get();
        return view('pages.battle', compact('game', 'bets', 'factor', 'bank', 'chances', 'tickets', 'lastwins'));
	}
	
	public function newGame() {
		$hash = bin2hex(random_bytes(16));
		
		DB::table('battle')->where('status', '<', 3)->update([
            'status' => 3
        ]);
	...
	public function newBet(Request $r) {
		...
		$color = $r->get('type');
		$sum = floor(preg_replace('/[^.0-9]/', '', $r->get('sum'))) ?? null;
	...
	public function getSlider() {
		...
		$this->game->commission 	= self::sendWinMoney($this->game, $winner);
        $this->game->save();
	...
	public function sendWinMoney($game, $color) {
		$comission = 0;
		...
		foreach($bets as $bet) {
			...
			if(!is_null($user)) {
				...
				$sum = floor($winmoney - ($winmoney * ($this->settings->battle_commission / 100)));
				$user->balance += $sum;
				$user->save();
