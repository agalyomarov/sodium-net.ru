$(document).ready(function() {
	...
	socket.on('jackpot.newBet', function(data) {
        ...
        for(var i = 0; i < data.chances.length; i++) {
            ...
            chances += '<td>' + data.chances[i].chance + '%/' + data.chances[i].sum + 'PT</td>';
            chances += '<td><button class="btn btn-primary btn-outline btn-xs" onclick="gotRoulette('+ room +',' + data.chances[i].id + ')"><i class="fa fa-check"></i></button></td></tr>';
        }
	...
	socket.on('roulette', (res) => {
        if(res.type == 'timer') $('#dtime').text((res.time > 9) ? res.time : '0'+res.time); 
        if(res.type == 'newGame') {
            $('#dtime').text(res.slider.time);
            $('#dlred').text(0);
            $('#dlgreen').text(0);
            $('#dlblack').text(0);
        }
        if(res.type == 'bets') {
			...
			{
				$('#dl' + color).text((typeof res.prices[color] == 'undefined') ? '0' : res.prices[color]);
			}
		}
    });
