...
<head>
  ...
  <meta content="{{$settings->desc}}" name="description">
  <meta content="{{$settings->keys}}" name="keywords">
  <meta name="viewport" content="width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=1">
  <meta content="{{ csrf_token() }}" name="csrf-token">
  <meta property="og:image" content="https://{{$settings->domain}}/assets/images/logo_m.png" />
  <link rel="icon" type="image/png" href="https://{{$settings->domain}}/assets/images/favicon.png">
  <link id="css_theme" rel="stylesheet" href="/assets/css/style_dark.css?v=32">
  <link rel="stylesheet" href="https://{{$settings->domain}}/assets/css/notifyme.css">
 <link rel="stylesheet" href="/assets/css/tooltipster.bundle.min.css?v=1">
<link href="https://fonts.googleapis.com/css?family=Open+Sans+Condensed:300&amp;display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.3.1/css/all.css"
    integrity="sha384-mzrmE5qonljUremFsqc01SB46JvROS7bZs3IO2EmfFsd15uHvIt+Y8vEf7N7fWAU" crossorigin="anonymous">
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/jquery-1.9.1.js"></script>
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/2.1.1/socket.io.js"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/jquery.slimscroll.min.js"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/jquery.cookie.js"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/jquery.kinetic.min.js"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/tooltipster.bundle.min.js?v=1"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/notifyme.min.js"></script>
  <script type="text/javascript" src="https://{{$settings->domain}}/assets/js/script.js?v=26"></script>
<script>
...
@auth
    <div class="user">
        ...
        <div class="money-block">
          ...
          <span id="money_mob" class="money-block__money-area">{{$u->balance}}</span>
 ...
 @if(Auth::check() && $u->is_admin)
        ...
        <div class="mobile-menu__submenu-item mobile-menu__submenu-item_social mobile-menu__submenu-item_last-in-row">
          <a href="{{$settings->vk_url}}" class="mobile-menu__submenu-item_social-link" target="_blank">
            <i class="myicon-vk"></i>
          </a>

        <a href="{{$settings->tg_url}}" class="mobile-menu__submenu-item_social-link" target="_blank">
    ...
    <ul class="leftside"> 
	  <li>
        <a href="/jackpot" title="Jackpot"
          class="tooltip-game mode {{ Request::is('jackpot') ? 'active' : '' }}">
      ...
      <li>
        <a href="/pvp" title="PVP"
          class="tooltip-game mode {{ Request::is('mines') ? 'active' : '' }}">
      ...
      <li>
        <a href="/double" title="Double"
          class="tooltip-game mode {{ Request::is('double') ? 'active' : '' }}">
          <i class="myicon-wheel"></i>
        </a>
      </li>

      <li>
        <a href="/battle" title="Battle" class="tooltip-game mode {{ Request::is('battle') ? 'active' : '' }}">
          <i class="myicon-sword"></i>
        </a>
      </li>

      <li>
        <a href="/crash" title="Crash" class="tooltip-game mode {{ Request::is('crash') ? 'active' : '' }}">
          <i class="myicon-graph"></i>
        </a>
      </li>

      <li>
        <a href="/dice" title="Dice" class="tooltip-game mode {{ Request::is('dice') ? 'active' : '' }}">
          <i class="myicon-dice"></i>
        </a>
      </li>
	  
	  <li>
        <a href="/raffle" title="Raffle" class="tooltip-game mode {{ Request::is('raffle') ? 'active' : '' }}">
      ...
      <li class="leftside__bottom-part">
        <a href="{{$settings->vk_url}}" title="Группа VK" target="_blank" class="mode mode_white tooltip-game">
          <i class="myicon-vk"></i>
        </a>
      </li>
      <li>
        <a href="{{$settings->tg_url}}" title="Группа Telegram" target="_blank" class="mode mode_white tooltip-game tooltipstered">
          <i class="myicon-telegram"></i>
        </a>
      </li>
      <li>
...
<div class="navbar">
 ...
 @auth
    <div class="balance">
      ...
      <span id="money_mob">{{$u->balance}}</span>
  ...
  <div class="user">
        ...
        <div class="profile">
      <img src="{{$u->avatar}}" class="profile-ava">
...
<div class="fixed-nav active">
 ...
 @if(Auth::check() && $u->is_admin)
    ...
    <li>
      <a href="/" class="mode {{ Request::is('jackpot') ? 'active' : '' }}">
        <i class="myicon-jackpot"></i>
        <div class="name">Jackpot</div>
        <span class="new">
          <b id="jackpot_bank">{{$gamesbank['jackpot']}}</b>
          <i class="myicon-coins"></i>
        </span>
      </a>
    </li>
    <li><a href="/double" class="mode {{ Request::is('double') ? 'active' : '' }}"><i class="myicon-wheel"></i><div class="name">Double</div><span class="new"><b id="double_bank">{{$gamesbank['double']}}</b> <i class="myicon-coins"></i></span></a></li>
    <li><a href="/battle" class="mode {{ Request::is('battle') ? 'active' : '' }}"><i class="myicon-sword"></i><div class="name">Battle</div><span class="new"><b id="battle_bank">{{$gamesbank['battle']}}</b> <i class="myicon-coins"></i></span></a></li>
    <li><a href="/crash" class="mode {{ Request::is('crash') ? 'active' : '' }}"><i class="myicon-graph"></i><div class="name">Crash</div><span class="new"><b id="crash_bank">{{$gamesbank['crash']}}</b> <i class="myicon-coins"></i></span></a></li>
    <li><a href="/dice" class="mode {{ Request::is('dice') ? 'active' : '' }}"><i class="myicon-dice"></i><div class="name">Dice</div></a></li>
...
@auth
  <li><a href="/" class="menu" rel="popup" data-popup="popup-pay"><i class="myicon-wallet"></i><div class="name">Пополнить</div></a></li>
    ...
    <li><a href="/top/partners" class="menu {{ Request::is('top/partners') ? 'active' : '' }}"><i class="myicon-team"></i><div class="name">Топ партнеров</div></a></li>
    <li><a href="/fair" class="menu {{ Request::is('fair') ? 'active' : '' }}"><i class="myicon-high-five"></i><div class="name">Честная игра</div></a></li>

        <li><a href="/pay/history" class="menu {{ Request::is('pay/history') ? 'active' : '' }}"><i class="myicon-clipboard"></i><div class="name">История счета</div></a></li>
    <li><a href="/ref" class="menu {{ Request::is('ref') ? 'active' : '' }}"><i class="myicon-collaboration"></i><div class="name">Реферальная система</div></a></li>
    <li><a href="/bonus" class="menu {{ Request::is('bonus') ? 'active' : '' }}"><i class="myicon-bonus"></i><div class="name">Бонусы</div></a></li>

    <li><a href="/faq" class="menu {{ Request::is('faq') ? 'active' : '' }}"><i class="myicon-info"></i><div class="name">FAQ</div></a></li>
    <li><a href="/rules" class="menu {{ Request::is('rules') ? 'active' : '' }}"><i class="myicon-documents"></i><div class="name">Правила использования</div></a></li>
    <li><a href="{{$settings->vk_url}}" target="_blank" class="menu"><i class="myicon-vk"></i><div class="name">Наша группа</div></a></li>
...
<img src="https://{{$settings->domain}}assets/images/favicon.png" style="display: none">
<div class="content">
...
<div class="fixed-chat ">
  ...
  <div class="messages">
    <div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 727px;"><div class="scroller" style="overflow: hidden; width: auto; height: 727px;">
      ...
      <div class="item" id="chatm_{{$sms['time2']}}">
        <div class="sub-heading clear">
          <div class="avatar"><img class="avatar-img" src="{{$sms['avatar']}}" alt=""></div>
          <div class="name" onclick="var u = $(this); $('.chat-input').val(u.text() + ', '); return false;">@if($sms['admin'])<span style="color:#ff5722;">[Администратор]</span> @elseif($sms['moder']) <span style="color:#4caf50;">[Модератор]</span> {{$sms['username']}} @elseif($sms['youtuber'])<span style="color:#ffc107;">[YouTube`r]</span> {{$sms['username']}} @else {{$sms['username']}} @endif</div>
          <div class="date">@if(Auth::user() and $u->is_admin == 1 || $u->is_moder == 1)<span onclick="var u = $(this); $('.chat-input').val('/ban ' + u.text() + ' '); return false;">{{$sms['user_id']}}</span> @else<span onclick="var u = $(this); $('.chat-input').val('/send ' + u.text() + ' '); return false;">{{$sms['user_id']}}</span>@endif @if(Auth::user() and $u->is_admin == 1 || $u->is_moder == 1)<a class="delete" onclick="chatdelet({{$sms['time2']}})"><i class="far fa-times-circle"></i></a>@endif</div>
        </div>
        <div class="text">{!!$sms['messages']!!}</div>
      </div>
      @endforeach @endif
        ...
        <table>
          ...
          <tbody id="promoshare">
            ...
            <tr data-id="{{$p->code}}">
              <td>{{$p->code}}</td>
              <td>{{$p->count_use}}</td>
              <td>{{$p->amount}} <i class="fas fa-coins"></i></td>
              <td><a class="btn" href="javascript:sharePromo('{{$p->code}}')">Поделиться в чате</a></td>
            </tr>
            @endforeach
          </tbody>
        </table>
