        ...
        <div class="user-profile-full-stats__content">
           ...
           <!--  <div class="user-profile-full-stats__error" style="display: none;">
                ...
                <tbody>
                    <tbody>
                        ...
                        <tr class="games-table__body-tr">            
                            <td class="games-table__body-td"><?php echo e($pay->id); ?></td>            
                            <td class="games-table__body-td">+<?php echo e($pay->price / 10); ?> ₽</td>            
                            <td class="games-table__body-td games-table__body-td_no_padding"> <?php if($pay->status == 1): ?> Пополнение баланса <?php elseif($pay->status == 2): ?> Реферальный код <?php else: ?> Промокод <?php endif; ?> </td>            
            ...
            <table class="games-table stats-table__withdraws games-table__withdraws" style="display: none;">
                ...
                <tbody>
                    <?php $__currentLoopData = $withdraws; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pay): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="games-table__body-tr">            
                        <td class="games-table__body-td"><?php echo e($pay->id); ?></td>            
                        <td class="games-table__body-td">-<?php echo e($pay->sum / 10); ?> ₽</td>            
                        <td class="games-table__body-td games-table__body-td_no_padding"> <?php echo e($pay->system); ?> </td>            
                        <td class="games-table__body-td <?php if($pay->status == 0): ?> status-pending <?php elseif($pay->status == 1): ?> status-success <?php elseif($pay->status == 2): ?> status-error <?php endif; ?>">
                            <span class="games-table__body-status">
                                <span class="myicon-success games-table__body-status-image"></span>
                                <span class="games-table__body-status-text"><?php if($pay->status == 0): ?> Обработка <?php elseif($pay->status == 1): ?> Выполнен <?php elseif($pay->status == 2): ?> Отменен <?php endif; ?></span>
                            </span>
                        </td>        
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
